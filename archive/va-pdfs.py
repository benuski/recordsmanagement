...
CONFIG = {
    ...
    "output_schema_path": "output_template_clean.json",
}
...
def make_record(schema: dict, **overrides) -> dict:
    ...
    Fields not in the schema (e.g. internal-only keys) are silently dropped.
    """
    record = dict(schema)  # shallow copy preserves all default empty values
...
def parse_using_marker_html_optimized(
    ...
    if html_path.exists():
        with open(html_path, 'r', encoding='utf-8') as f:
            html_content = f.read()
        return parse_using_marker_html(html_content, schedule_id, effective_date, agency_name, schema)
...
def process_and_evaluate(pdf_path: Path, output_dir: Path, agency_mapping: dict, schema: dict) -> None:
        ...
        for strategy in strategies:
            ...
            if strategy == 'html':
                records = parse_using_marker_html_optimized(
                    pdf_path, schedule_id, effective_date, agency_name, is_image, schema
                )
            elif strategy == 'table':
                records = parse_using_table_engine(
                    pdf_path, schedule_id, effective_date, agency_name, schema
                )
            elif strategy == 'silo':
...
if __name__ == "__main__":
    ...
    schema_path = Path(CONFIG["output_schema_path"])
